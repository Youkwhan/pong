...
function love.update(dt)
    ...
    --
    -- PLAYER KEYBOARD paddles can move no matter what state we're in
    --
    -- player 1
    ...
    --(ADD PLAYER AI in respect to the balls DY cords)
    -- update our ball based on its DX and DY only if we're in play state;
    -- scale the velocity by dt so movement is framerate-independent
    if gameState == 'play' then
        --Player 1 AI
        --if the balls x velocity is negative, left player has to move
        if ball.dx < 0 then
            --if ball is above paddle.y and a little bit more move up 
        ...
        --Player 2 AI
        --if balls x velocity is positive the right side needs to recieve
        if ball.dx > 0 then
